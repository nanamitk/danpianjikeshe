...
KEY:     MOV   A,P1		            ;屏蔽高4位，判断是否有键按下			
         ANL   A,#0FH              	; ANL 为于指令 书本P48	  该功能为将A值的高四位过滤 

         JNB   ACC.0,DD          ;判A中的位单元，按下则跳转DD	 JNB ：如果前参数不为1 则跳转到后参数位置	书本P57
         JNB   ACC.1,DD
         JNB   ACC.2,DD
         JNB   ACC.3,DD
         SJMP  KEY					 ;循环该程序KEY
...
DSINIT:  MOV   DPTR,#7F0BH          ;初始化寄存器B，禁止芯片内部的更新周期操作，即SET位置1	 （7F0B为芯片中寄存器B的地址）
	 MOV   A,#82H					;82为1000 0010 书本P166 寄存器B的说明
	 MOVX  @DPTR,A
     ...
     MOV   DPTR,#7F06H          ;初始化星期为05
	 MOV   A,#05H
     MOVX  @DPTR,A

     MOV   DPTR,#7F07H          ;初始化日为18
	 MOV   A,#18H
     MOVX  @DPTR,A
